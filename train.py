...
def get_train_loader(config, logger):
    ...
    train_dataset = eval(dataset_name)(config, split=split, logger=logger)
...
def main(args):
    ...
    ################## optimizer ##################
    layer_decay = config.TRAIN.get("LAYER_DECAY", 1.0)
    optimizer, scheduler = create_optimizer(config, model)
    ...
    for epoch in range(config["TRAIN"]["BEGIN_EPOCH"], config["TRAIN"]["END_EPOCH"]):
        ...
        if (epoch + 1) % save_interval == 0 or epoch == config["TRAIN"]["END_EPOCH"] - 1:
            ...
            model_path_final_epoch = os.path.join(
                config["OUTPUT_DIR_TRAIN"],
                f"{config['CONFIG_NAME'].split('_')[0]}-final-e{epoch+1}-train-{train_loss:.3f}-val-{val_loss:.3f}.pt",
            )
            torch.save(
                {
                    "epoch": epoch + 1,
