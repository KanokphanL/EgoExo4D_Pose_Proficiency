...
def val(
    ...
    return val_loss, accuracy, test_result_json
...
def main(args):
    ...
    ################## validation ##################
    val_loss, accuracy, result_dict = val(
        config,
        device,
        val_loader,
        model,
    ...
    loss_name = config["VAL"]["LOSS_CRITERION"]
    logger.info(f"Val {loss_name}: {val_loss:.4f}, acc: {accuracy:.4f}")

    ################## save result ##################
    model_name = os.path.splitext(os.path.basename(model_path))[0]
    test_result_json_name = f"val_{model_name}_val-acc-{accuracy:.3f}.json"
    save_path = os.path.join("val_output", test_result_json_name)
    ...
    accuracy_after = calculate_accuracy(
        ...
        groundtruth)
    logger.info(f"Val set accuracy after poseprocess by scores:  {accuracy_after:.4f}")
    ...
    logger.info(f"Val set accuracy after poseprocess by voting:  {accuracy_after:.4f}")
    ...
    model_name = os.path.splitext(os.path.basename(model_path))[0]
    result_json_name = f"val_postprocessing_{model_name}_val-acc-{best_accuracy:.3f}.json"
