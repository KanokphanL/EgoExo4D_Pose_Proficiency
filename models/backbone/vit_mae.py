...
# @register_model
def vit_small_patch16_224(pretrained=None, all_frames=16, **kwargs):
...
# @register_model
def vit_base_patch16_224(pretrained=None, all_frames=16, **kwargs):
    model = VisionTransformer(
        patch_size=16,
        embed_dim=768,
        depth=12,
...
if __name__ == "__main__":
    ...
    num_frames = 5
    C = 3
    input_tensor = torch.randn(batch_size, C, num_frames, H, W)

    print("Input shape:", input_tensor.shape)

    pretrained = "ckpts/vit_mae/vit_s_k710_dl_from_giant.pth"

    model = vit_small_patch16_224(pretrained=pretrained, all_frames=num_frames)
    model.head_dropout = torch.nn.Identity()
